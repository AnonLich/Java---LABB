...
/**
 * A simple program where the user can sketch curves in a variety of colors. A
 * color palette is shown along the right edge of the canvas. The user can
...
public class Main extends Application {
	...
	/*
	 * Array of colors corresponding to available colors in the palette. (The last
	 * color is a slightly darker version of yellow for better visibility on a white
	 * background.)
	 */
	private final Color[] palette = { Color.BLACK, Color.RED, Color.GREEN, Color.BLUE, Color.CYAN, Color.MAGENTA,
	...
	/**
	 * The start() method creates the GUI, sets up event listening, and shows the
	...
	public void clearAndDrawPalette() {
		...
		/*
		 * Draw a 3-pixel border around the canvas in gray. This has to be done by
		 * drawing three rectangles of different sizes.
		 */
		...
		/*
		 * Draw a 56-pixel wide gray rectangle along the right edge of the canvas. The
		 * color palette and Clear button will be drawn on top of this. (This covers
		 * some of the same area as the border I just drew.
		 */
		...
		/*
		 * Draw a 2-pixel white border around the color rectangle of the current drawing
		 * color.
		 */
	...
	/**
	 * Change the drawing color after the user has clicked the mouse on the color
	 * palette at a point with y-coordinate y.
	 */
	private void changeColor(int y) {
		...
		/*
		 * Remove the highlight from the current color, by drawing over it in gray. Then
		 * change the current drawing color and draw a highlight around the new drawing
		 * color.
		 */
	...
	/**
	 * This is called when the user presses the mouse anywhere in the canvas. There
	 * are three possible responses, depending on where the user clicked: Change the
	 * current color, clear the drawing, or start drawing a curve. (Or do nothing if
	 * user clicks on the border.)
	 */
	...
	/**
	 * Called whenever the user releases the mouse button. Just sets dragging to
	 * false.
	 */
	 ...
	 * Note that in case the user drags outside of the drawing area, the values of x
	 * and y are "clamped" to lie within this area. This avoids drawing on the color
